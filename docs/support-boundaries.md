...
- **Inotify watch limits on Linux.** A large tree needs one watch per directory. When the kernel's `fs.inotify.max_user_watches` runs out — at startup, or later as new directories appear — the watcher reports it once and the session falls back to interval polling, rebuilding the watch every 30 seconds. Polling still converges but no longer reacts within a fraction of a second. Raise the limit with `sysctl` and persist it.
