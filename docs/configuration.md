...
| `disabled` | list of hosts | `[]` | Hosts excluded everywhere. A disabled beta drops that beta; a disabled *alpha* drops the whole group. |
...
`disabled` and `log` are valid nowhere else, so the same slip is caught.

## Session settings

Thirteen keys live in both `[defaults]` and any group, with the group
winning. Three exist only on a group.
