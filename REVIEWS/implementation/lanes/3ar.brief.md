...
- **Your worktree:** the current directory. It is a git worktree on your own branch, `lane-3ar`. Commit there only. Never switch branches, never create branches or worktrees, never rebase, and never push. There is no remote.
...
2. **For each ticket, in order:**
   ...
   4. Run `cargo fmt`, then `cargo clippy --release --all-targets -- -D warnings`, then the tests you touched plus the module's existing tests. Always run tests through the isolation wrapper: `isolated lane-3ar cargo test --release <filter>`. It gives tests a private HOME, and it must be used for every test run. Before your final commit, run the whole suite once: `isolated lane-3ar cargo test --release --no-fail-fast`.
...
6. **Keep your notes current.** Append short progress notes to `/home/ubuntu/lanes/3ar.notes.md` as you go: decisions, departures from the ticket, anything the integrator must know.
7. **When you are finished,** write `/home/ubuntu/lanes/3ar.done.md` with:
...
## Your job: replay lane 3a's finished work onto the current code

Lane `3a` finished its tickets on branch `lane-3a`, which started from tag `cf70a91`. Since then, other lanes' work has landed on `integration`, and replaying `lane-3a` onto it conflicts. Your worktree is a new branch `lane-3ar`, starting from the current `integration`.

1. Read `/home/ubuntu/lanes/3a.done.md` and `/home/ubuntu/lanes/3a.notes.md`, and the tickets they name.
2. Replay the commits of `git rev-list --reverse cf70a91..lane-3a` one at a time, with `git cherry-pick -x <commit>`.
...
4. **After each commit,** run `cargo fmt`, clippy, and the tests for the touched modules, using `isolated lane-3ar cargo test --release <filter>`. If a resolution needs a follow-up fix, amend it into that cherry-pick's commit.
5. **Finish** with the full suite: `isolated lane-3ar cargo test --release --no-fail-fast`. Write `/home/ubuntu/lanes/3ar.done.md`, listing each replayed commit, how each conflict was resolved, and the final test results.
