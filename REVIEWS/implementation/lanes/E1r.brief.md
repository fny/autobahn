...
- **Your worktree:** the current directory. It is a git worktree on your own branch, `lane-E1r`. Commit there only. Never switch branches, never create branches or worktrees, never rebase, and never push. There is no remote.
...
2. **For each ticket, in order:**
   ...
   4. Run `cargo fmt`, then `cargo clippy --release --all-targets -- -D warnings`, then the tests you touched plus the module's existing tests. Always run tests through the isolation wrapper: `isolated lane-E1r cargo test --release <filter>`. It gives tests a private HOME, and it must be used for every test run. Before your final commit, run the whole suite once: `isolated lane-E1r cargo test --release --no-fail-fast`.
...
6. **Keep your notes current.** Append short progress notes to `/home/ubuntu/lanes/E1r.notes.md` as you go: decisions, departures from the ticket, anything the integrator must know.
7. **When you are finished,** write `/home/ubuntu/lanes/E1r.done.md` with:
...
## Your job: replay lane E1's finished work onto the current code

Lane `E1` finished its tickets on branch `lane-E1`, which started from tag `wave-0`. Since then, other lanes' work has landed on `integration`, and replaying `lane-E1` onto it conflicts. Your worktree is a new branch `lane-E1r`, starting from the current `integration`.

1. Read `/home/ubuntu/lanes/E1.done.md` and `/home/ubuntu/lanes/E1.notes.md`, and the tickets they name.
2. Replay the commits of `git rev-list --reverse wave-0..lane-E1` one at a time, with `git cherry-pick -x <commit>`.
...
4. **After each commit,** run `cargo fmt`, clippy, and the tests for the touched modules, using `isolated lane-E1r cargo test --release <filter>`. If a resolution needs a follow-up fix, amend it into that cherry-pick's commit.
5. **Finish** with the full suite: `isolated lane-E1r cargo test --release --no-fail-fast`. Write `/home/ubuntu/lanes/E1r.done.md`, listing each replayed commit, how each conflict was resolved, and the final test results.
