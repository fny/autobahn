...
fn stream_agent(destination: &str, content: &[u8], compressed: Option<&[u8]>) -> Result<()> {
    ...
    send_agent(
        ...
        compressed.unwrap_or(content).to_vec(),
        compressed.is_some(),
    )
}

/// Runs the installation command, streaming the binary to it within its
/// deadline, and reports its failure in its own words. With `compressed`,
/// exit 71 is the remote saying it has no gzip, and the caller sends the
/// binary as it is instead.
fn send_agent(command: Command, content: Vec<u8>, compressed: bool) -> Result<()> {
    let timeout = upload_timeout(content.len());
    let output = run_within(command, Some(content), timeout, "the agent upload")?;
    if compressed && output.status.code() == Some(71) {
...
mod tests {
    ...
    fn a_failed_upload_says_what_the_remote_said() {
        ...
        command
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
        let error = send_agent(command, vec![0; 8 << 20], false).expect_err("the upload fails");
    ...
    fn the_remote_command_runs_under_sh_and_picks_this_platform() {
        ...
        // and that nothing was picked, which is the claim the first half of
        // this test makes and the part a third platform cannot change.
        assert!(
