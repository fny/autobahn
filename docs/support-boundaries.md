...
Roots must be directories. A single file cannot be a root; sync its
parent and ignore the rest.
...
- **Live databases and other multi-file formats.** A SQLite database is
  ...
  than the live file.
- **Inotify watch limits on Linux.** A large tree needs one watch per
  directory. When the kernel's `fs.inotify.max_user_watches` runs out
  the watcher reports it and the session falls back to interval polling,
  which still converges but no longer reacts within a fraction of a
  second. Raise the limit with `sysctl` and persist it.
