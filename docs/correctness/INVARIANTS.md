...
- `offer_baseline` refuses an offer based on an older generation than the standing baseline, so a stale fold can never roll the baseline back past a change whose dirty marks a scan already consumed. A transition offers its fold at the generation of the lease it was built from, not the observer's current one (finding H-3), and moves the generation it has seen past its own announcements only when nothing else advanced it in between, so a foreign change during a transition still wakes the session.
...
**Checked by**: `a_mid_scan_change_is_never_served_as_current`, `an_offered_baseline_cannot_hide_an_invalidated_change`, the randomized interleaving sweep `every_interleaving_scans_the_truth` (all in `src/endpoint/observer.rs`), `a_scan_racing_a_transition_cannot_outlive_the_writes` (`src/endpoint/local.rs`), which pauses a real transition between its announcement and its writes under the suppressed-watcher seam, and `a_stale_lease_cannot_roll_back_a_sharing_sessions_deletion_when_watched` (with its `_when_polled` twin) and `a_foreign_change_during_a_transition_still_wakes_the_session` (`src/endpoint/local.rs`), which drive two real endpoints over one observer through `scan` and `transition` only. The protocol properties are mutation-checked: disabling the path recording, the offer refusal, the serve gate, or the post-write announcement turns a test red. The sweep found two real holes on its first runs, and the independent review found a third (I1-A) — all fixed.
...
- Corruption anywhere is fail-closed: an undecodable journal is never a silent reset. The session rebuilds the ancestor only when both sides already match (`Session::rebuild_or_halt` — reconciliation with no ancestor yields no transitions and no conflicts, so no history could have changed the outcome), setting the unreadable one aside; otherwise it halts (`SafetyHalt::AncestorUnreadable`). Damage is rebuilt once per session, then halts (`AncestorDamagedAgain`) until a reset. Checked by the `unreadable_ancestor` e2e tests. A journal record's encoding is the checkpoint format's (`decode_record`), held to fixed bytes by `the_encodings_this_format_promises_are_unchanged`.
...
**Checked by**: the reconcile property suite (`conflict_free_reconciliation_converges`, `agreement_emits_nothing`, `unsynchronizable_content_never_travels`, `mutual_exclusion_preserves_the_ancestor` — mutation-checked), `concurrent_divergent_edits_conflict_in_safe_mode_and_resolve_in_resolved_mode`, `deletion_versus_modification_repropagates_content`, `one_way_safe_preserves_beta_creations`, `one_way_modes_never_touch_alpha`, `content_leaving_tracked_scope_never_reads_as_deletion`, `emptied_root_detection`, `a_root_emptied_down_to_an_ignored_entry_halts_in_every_mode` (mutation-checked), `no_change_is_lost_silently`, `paranoid_treats_an_emptied_large_directory_as_a_conflict`, `paranoid_treats_a_directory_emptied_down_to_an_ignored_entry_as_emptied`, `paranoid_restores_a_large_directory_gone_from_one_side`, `paranoid_lets_the_emptying_side_win_once_the_full_copy_is_retired`.

**Boundary.** A vanished mount holding fewer than eight entries — one huge file — evades the count guard: RETAINED.md §1. A root emptied down to one empty *directory* still counts as not emptied: an empty directory synchronizes, and telling that shape from a deliberate clear-out would need a threshold rather than a yes/no. The guard also presumes trustworthy provenance; a fabricated ancestor (I2's hostile- agent boundary) makes safe-mode arithmetic destructive (finding I7-B).
